# DOC BRIEF – 문서 요약 서비스 (2인 프로젝트)
- 다양한 문서 포맷을 파싱하여 문단/문장 단위로 구조화하고,
이를 기반으로 요약 요청까지 연결하는 백엔드 중심 문서 처리 프로젝트<br/>

![DocBrief](https://github.com/user-attachments/assets/788766c0-6ca5-4633-8bcc-02ae47329004)

## 💻 1.프로젝트 소개
- 파일 또는 URL 기반 문서를 업로드
- 문서 파싱(TXT, PDF, DOCX) 문단 / 문장 단위로 구조화
- 문서 요약 결과 보관함 저장

## 🕑 2.개발기간
- 2024/01 (2주)

## ⚙ 3.개발환경
Backend
- 개발언어 : <img src="https://img.shields.io/badge/Java8-000000?style=flat&logo=openjdk&logoColor=white"/>
- 프레임워크 : <img src="https://img.shields.io/badge/Spring Boot-6DB33F?style=flat&logo=springboot&logoColor=white"/>
- DB : <img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white"/>
- ORM/Persistence : <img src="https://img.shields.io/badge/JPA-Hibernate-59666C?style=flat"/>

Frontend
- <img src="https://img.shields.io/badge/vue.js-4FC08D?style=flat&logo=vue.js&logoColor=white"> 

## 👉 4.주요기능
<img width="1346" height="790" alt="Image" src="https://github.com/user-attachments/assets/6a689d9c-070f-4fdf-83c9-447fd8c572eb" />
- - -<br/>
<img width="1681" height="861" alt="Image" src="https://github.com/user-attachments/assets/8a06080f-e0d3-4081-9bd0-3b73bd20bd32" />
- - -<br/>
<img width="912" height="611" alt="Image" src="https://github.com/user-attachments/assets/bbea66e8-1a1a-451b-b75c-13b30db94e51" />



## 📃 5. 산출물
- ERD 다이어그램
<img width="1180" height="553" alt="Image" src="https://github.com/user-attachments/assets/5b595103-e5c4-4d7a-af17-2dffed7c0e92" />
